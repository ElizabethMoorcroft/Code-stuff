...
classifcation.simulation.validation<-function(data,seed,name){
  ...
  temp<-matrix(ncol=11,nrow=length(unlist(data,recursive=FALSE)))
  colnames(temp)<-c("N","Sex","NoCluster","AnimalsInTraining","NoOfTrainingLocations","AnimalsInValidation","NoOfValidationLocations","BIC","PvalueDistbutionOfDisplacement","PValueDistributionOfAngle","PvalueOfSpaceUse")
  ...
  for(i in all.block.numbers){
    ...
    pdf(paste(name,"_block",i,"_sex",sex,".pdf",sep=""))
    par(mfrow=c(2,2))
    ...
    temp[i,4]<-paste(unique(all.blocks$AnimID),collapse = ",");temp[i,5]<-sum(!is.na(all.blocks$block_angle.difference))
    ...
    temp[i,6]<-paste(unique(all.blocks.valid$AnimID),collapse = ",");temp[i,7]<-sum(!is.na(all.blocks.valid$block_angle.difference))
...
data.for.classifcation.spring.50<-create.blocks(Data,no.of.miss.occasions=4,min.no.locations=50,start.month=1,Noofmonths=3)
data.for.classifcation.winter.50<-create.blocks(Data,no.of.miss.occasions=4,min.no.locations=50,start.month=10,Noofmonths=3)
data.for.classifcation.summer.50<-create.blocks(Data,no.of.miss.occasions=4,min.no.locations=50,start.month=4,Noofmonths=3)
...
class.winter.35<-classifcation.simulation.validation(b.w35,seed=1,"Winter_35locs_")

b.sum35<-data.for.classifcation.summer.35[[2]]
class.summer.35<-classifcation.simulation.validation(b.sum35,seed=1,"Summer_35locs_")

b.a35<-data.for.classifcation.autumn.35[[2]]
class.autumn.35<-classifcation.simulation.validation(b.a35,seed=1,"Autumn_35locs_")


b.s50<-data.for.classifcation.spring.50[[2]]
class.spring.50<-classifcation.simulation.validation(b.s50,seed=1,"Spring_50locs_")

b.w50<-data.for.classifcation.winter.50[[2]]
class.winter.50<-classifcation.simulation.validation(b.w50,seed=1,"Winter_50locs_")
